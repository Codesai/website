...
| CWS-DEP-001 | DONE | P1 | Ruby/Calidad | Actualizar `html-proofer` de 5.2.1 a 5.2.2 y `async` de 2.6.5 a una versión actual compatible con Ruby 4, eliminando el pin heredado. | `bundle outdated --strict` no muestra versiones pendientes dentro del alcance; `make test` pasa; HTML-Proofer comprueba todos los enlaces sin avisos `Scheduler should implement #fiber_interrupt`. |
| CWS-DEP-002 | DOING | P1 | JavaScript/Pruebas | Actualizar `@playwright/test` de 1.55.1 a 1.62.1 y regenerar `package-lock.json` y los binarios de Chromium. | `npm outdated` no muestra Playwright pendiente y las pruebas de escritorio/móvil pasan en ES/EN con la nueva versión. |
| CWS-DEV-001 | DONE | P1 | Desarrollo | Adaptar `scripts/development_build.sh` a Bundler 4, ejecutando Jekyll mediante `bundle exec`. | El script arranca Jekyll sin depender de un ejecutable global y el servidor responde HTTP 200 en `localhost:4000`. |
| CWS-SYS-001 | DOING | P1 | Docker/Sistema | Migrar la imagen base de `ruby:4.0.6-bookworm` (Debian 12 oldstable) a `ruby:4.0.6-trixie` (Debian 13 stable). | La imagen compila para amd64 y arm64; Ruby, Node, optimizadores de imágenes y Playwright funcionan; `make test` pasa completo. |
