...
1. **Red de seguridad:** `CWS-TEST-001` y `CWS-TOOL-007`.
2. **Correcciones críticas:** `CWS-A11Y-001`, `CWS-LH-001` y `CWS-LH-002`.
3. **Semántica e interacción:** `CWS-A11Y-002`, `CWS-A11Y-003`, `CWS-A11Y-004` y `CWS-JS-001`.
4. **Peso y estabilidad visual:** `CWS-IMG-002`, `CWS-IMG-001`, `CWS-ICON-001` y `CWS-FONT-001`.
5. **Privacidad y prevención de regresiones:** `CWS-ANA-001`, `CWS-CI-001`, `CWS-LH-003` y `CWS-TOOL-004`.
6. **Validación externa y datos reales:** `CWS-TOOL-003`, `CWS-TOOL-001` y `CWS-TOOL-002`.
7. **Mejoras de prioridad media:** `CWS-IMG-003`, `CWS-CACHE-001`, `CWS-SEC-001`, `CWS-TOOL-005`, `CWS-TOOL-006` y `CWS-TOOL-008`.

## Explicación del orden

La primera fase crea una línea base funcional antes de tocar HTML, JavaScript, imágenes o estilos. Así, cada cambio posterior puede demostrar que el sitio sigue compilando, que los enlaces y recursos internos existen, que las rutas desconocidas continúan devolviendo 404 y que menú, idioma, cookies y formulario conservan su comportamiento en ES/EN y en móvil/escritorio.

Después se atienden los fallos P0 con mayor impacto directo: accesibilidad y LCP/FCP móvil. Las mejoras semánticas y la retirada de JavaScript se ejecutan a continuación porque afectan controles interactivos y se benefician especialmente de la cobertura Playwright creada al inicio. La optimización masiva de imágenes, iconos y fuentes queda después, ya que tiene un alcance transversal y necesita controles funcionales y visuales estables.

Por último se automatizan los umbrales en CI, se repiten las mediciones controladas y se contrastan con herramientas externas y datos de campo. Caché, cabeceras, rastreo SEO, datos estructurados y regresión visual se mantienen como P2 porque aportan valor, pero no deben retrasar la red de seguridad ni las correcciones críticas ya demostradas por Lighthouse.
