...
date: 2017-07-18 00:00:00.0 +00:00
type: post
published: true
status: publish
tags:
...
In some languages, such as Ruby, Clojure, C#, Python, etc, this type of method can be refactored by introducing *named parameters* (see [Introduce Named Parameter](http://refactoring.com/catalog/introduceNamedParameter.html) refactoring)<a href="#nota1"><sup>[1]</sup></a>.
...
Well, it depends. There're some trade-offs to consider. *Positional parameters* produce more succint calls and depending on the method name the intent of the arguments might be easy to guess. Using *named parameters* gives us better code clarity and maintainability than *positional parameters*, but we lose terseness<a href="#nota2"><sup>[2]</sup></a>. 

However, we should also consider the influence of the **degree and locality of each instance of CoP**<a href="#nota3"><sup>[3]</sup></a> on the maintainability and communication of intent of each option. On one hand, the impact on maintainability of using *positional parameters* is higher for public methods than for private methods (even higher for [published public methods](https://martinfowler.com/bliki/PublishedInterface.html))<a href="#nota4"><sup>[4]</sup></a>. On the other hand, I think, that a similar reasoning can be made about the intent of *positional parameters*. The *positional parameters* of a private method in a cohesive class will probably be much easier to understand than for the parameters of a public method of a class we are using, because in the former case we have much more context to help us understand. 

The communication of intent of using *positional parameters* can be improved a lot with the [parameter name hinting feature](https://www.youtube.com/watch?v=ZfYOddEmaRw) provided by IDEs like IntelliJ. In any case, even though they look like *named parameters*, they still are *positional parameters* and have CoP. In this sense, *parameter name hinting* might end up having a bad effect in your code by reducing the pain of having [long parameter lists](http://www.informit.com/articles/article.aspx?p=1400866&seqNum=).
...
We have eliminated the CoP and now it only has CoN, right?

In this particular case, the answer is **no**. We're just masking the real problem which was a **Connascence of Meaning (CoM)** (a.k.a. **Connascence of Convention**). CoM happens **when multiple components must agree on the meaning of specific values**<a href="#nota4"><sup>[4]</sup></a>. CoM is telling us that there might be **a missing concept or abstraction in our domain**. In this case, we were facing a case of *data clump*, that we can eliminate by introducing a *Range* [value object](https://martinfowler.com/bliki/ValueObject.html) to reify the missing concept in our domain. This refactoring plus the introduction of named parameters really reduces the connascence from CoP to CoN:

<script src="https://gist.github.com/trikitrok/5f3b86f7cb59cadc8f2a3ac962090fef.js"></script>

This refactoring is way better than only introducing *named parameters* because it does not only provides a **bigger coupling reduction** by goes down in the scale from from CoP to CoN instead of only from CoP to CoM, but also it **introduces more semantics by adding a missing concept** (the *Range* value object) that will attract behavior.

### Summary.
We have presented two examples of CoP, a "pure" one and another one that was hiding a case of CoM. We have related CoP and CoM with known code smells, (such as, [Long Parameters List](http://www.informit.com/articles/article.aspx?p=1400866&seqNum=4), [Data Clump](http://www.informit.com/articles/article.aspx?p=1400866&seqNum=8) and [Primitive Obsession](http://www.informit.com/articles/article.aspx?p=1400866&seqNum=9)), and introduced refactorings to reduce their coupling and improve their communication of intent. We have also discussed a bit, about when and what we need consider before applying these refactorings.
...
<div class="foot-note">
  <a name="nota4"></a> [4] For instance, <a href="https://www.sandimetz.com/"> recommends in her <a href="http://www.poodr.com/">POODR book</a> to "use hashes for initialization arguments" in constructors (this was the way of having named parameters before <a href="https://robots.thoughtbot.com/ruby-2-keyword-arguments">Ruby 2.0 introduced keyword arguments</a>).
</div>

<div class="foot-note">
  <a name="nota4"></a> [4] <a href="http://www.informit.com/articles/article.aspx?p=1400866&seqNum=8">Data Clump</a> and <a href="http://www.informit.com/articles/article.aspx?p=1400866&seqNum=9">Primitive Obsession</a> smells are examples of CoM.
</div>
