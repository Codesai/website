...
ya que nos veríamos forzados a cambiar también los test. Este incremento en el coste obstaculizará la refactorización.

Muchos problemas al usar dobles de prueba son causados únicamente por esta idea errónea. Afortunadamente, esta idea parece estar retrocediendo, 
ya que cqada vez más desarrolladores están empezando a entender que la unidad en los test unitarios es el comportamiento y no la clase. 
...
#### La idea errónea que persiste: considerar que la clase es la unidad en el estilo de TDD de la Escuela de Londres o estilo Mockist.
...
Desafortunadamente, esta otra idea errónea está muy extendida, y está provocando que, o bien, algunos desarrolladores apliquen el estilo mockist de TDD de manera completamente incorrecta 
...
En el resto de nuestra discusión, usaremos el libro [Growing Object Oriented Software, Guided by Tests](http://www.growing-object-oriented-software.com/) (GOOS) de [Steve Freeman](https://www.linkedin.com/in/stevefreeman) y [Nat Pryce](https://www.linkedin.com/in/natpryce/) como la referencia para el estilo mockist de TDD.

## La clase no es la unidad en el estilo mockist de TDD.
...
Como indica su título, en la sección *Don’t Mock Values* (capítulo 20: *Listening to the Tests*) se recomienda no usar dobles de prueba para simular valores en los tests.
...
Esta recomendación por sí sola demuestra que la idea de aislar la clase que estamos probando de todas las clases con las que colaboran nunca ha formado parte del estilo mockist.
...
Hay autores que aún reconociendo la distinción entre  **valores** y **objetos**, siguen sosteniendo que la clase es la unidad en el estilo mockist de TDD<a href="#nota4"><sup>[4]</sup></a>. Acabamos de mostrar que dicha afirmación es contradictoria.

Hay más ideas en el libro GOOS que refutan esta falsa creencia de considerar que la clase es la unidad en el estilo mockist de TDD.

#### Prueba 2: Objetos Internos frente a Pares (“colaboradores reales”).
...
"We should [only] mock an object’s peers—its dependencies, notifications, and adjustments [...]—but not its internals" (“Deberíamos simular [sólamente] los pares de un objeto, sus dependencias, notificaciones y ajustes [...], pero no sus objetos internos”) (capítulo 7: *Achieving Object-Oriented Design*).
...
Al igual que el estilo clásico de TDD, el estilo mockist de TDD considera **el comportamiento como la unidad** en los tests unitarios<a href="#nota5"><sup>[5]</sup></a>. Y, como hemos visto, un comportamiento puede ser implementado por 1 o N clases, pero esto último ya es sólamente un detalle de implementación.
...
<a name="nota1"></a> [1] Cuando probamos clases aisladas, los tests resultantes tendrán una sensibilidad a la estructura muy alta, cuando lo ideal sería que los tests sean insensibles a la estructura (ver [Test Desiderata 2/12 Tests Should be Structure-Insensitive](https://www.youtube.com/watch?v=bvRRbWbQwDU)).
...
<a name="nota5"></a> [5] Los autores del libro GOOS afirman que debemos “Unit-Test Behavior, Not Methods” (“Probar el comportamiento, no los métodos”) (capítulo 5: *Maintaining the Test-Driven Cycle*).
